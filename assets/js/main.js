...
function toggleMobileDd(id) {
    const toggle = document.getElementById('dd-toggle-' + id);
    const items = document.getElementById('dd-items-' + id);
...
function triggerInquiry(name) {
    window.open(`https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(`Hello Benita Emmanuel, I am interested in enrolling for the ${name} program. Please send me details.`)}`, '_blank');
}
...
function submitBooking() {
    ...
    window.open(`https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(`*Booking Request*\n\nHi Benita, I'd like to book the *${track.value}* starting around *${date.value}*. Let's discuss timings.`)}`, '_blank');
    closeBookingModal();
...
document.addEventListener('click', function(e) {
    ...
    if (menu && !menu.classList.contains('hidden') && !navEl.contains(e.target)) {
